export default async function handler(req, res) {
  ...
  try {
    ...
    // Anthropic形式の messages 配列からユーザーのテキストを抽出
    let userPrompt = '';
    if (Array.isArray(messages)) {
      userPrompt = messages.map(m => m.content).join('\n');
    } else {
      userPrompt = messages || '神託を授けてください。';
    }
    ...
    const generatedText = data.candidates?.[0]?.content?.parts?.[0]?.text || '神託を取得できませんでした。';

    // App.jsx が期待している { content: [{ text: '...' }] } の形式で返す
